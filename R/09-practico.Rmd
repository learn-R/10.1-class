...
### Regresión con paquete [`stats`](https://www.rdocumentation.org/packages/stats/versions/3.6.2)
...
```{r include=FALSE}
#quitar esto cuando se actualicen los datos
datos$ciuo08 <- as.numeric(datos$ciuo08)
datos$sexo <- as.numeric(datos$sexo)
datos$est_conyugal <- as.numeric(datos$est_conyugal)

datos <- datos %>% filter(edad>1, ing_t_t>1)
```


Para modelos log-lineal
...
¡Pueden hacer la transformación funcional que estimen conveniente!

 Para la creación exploratoria
...
sjPlot::tab_model(modelo0, show.ci=FALSE,  encoding = "UTF-8", file = "output/figures/regnc_tab.doc")
```
...
Para esto con el argumento `type`, podemos especificar los calores predichos para los **términos** específicos del modelo
...
Y para especificar los términos se utiliza el argumento `terms`, lo que indica los términos a mostrar en los efector marginales
...
Debe mencionarse al menos uno y máximo tres

```{r message=FALSE, warning=FALSE}
plot_model(modelo5_log,
           type = "pred",
           terms = c("sexo", "est_conyugal")) 
