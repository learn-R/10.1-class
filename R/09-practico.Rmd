...
Ahora sabemos que trabajaremos con `"ingresos"`,  `"educacion"`, `"sexo"` y `"edad"`. Ahora inclusive podemos repasar lo visto en el [práctico anterior](https://learn-r-udp.netlify.app/example/04-practico/)  y explorar nuestros datos con `sjPlot::view_df()`
...
¡Genial! ahora que repasamos los tipos de variables y sabemos que estadísticos son para cada tipo pasaremos al **análisis bivariado**

# 4. Análisis bivariado

Es relevante que previo a la regresión lineal se realicen análisis bivariados, esto pueden encontrarlo con más detalle en el [práctico de descriptivos](https://learn-r-udp.netlify.app/example/04-practico/). En esta ocasión repasaremos con una **tabla de correlación**, **un gráfico de puntos** y **un gráfico de cajas**


## 4.1 Correlación

Para generar una tabla de correlación entre las variables `ingresos` y `edad`,  usaremos la función `tab_corr` de `sjPlot`
...
```{r ESI corr, echo=TRUE, results='markup'}
datos_proc %>%
      select(ingresos, edad) %>% 
      tab_corr(.,
          triangle = "lower",   
...
Ahora podemos observar que efectivamente existe una correlación entre los ingresos y la edad.

## 4.2 Gráfico de nube de puntos (scatter)

Podemos graficar la relación de ambas variables con la función `plot_scatter` de `sjPlot`

```{r ESI scatter err, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
datos_proc %>%  
   plot_scatter(., ingresos, edad)
```


Acá nos damos cuenta de que la variable `ingresos`, contiene casos muy concentrados en los niveles más bajos y pocos casos muy altos, para visualizarlos de mejor forma, los filtraremos (recuerden que `sjPlot` es compatible con el universo `tidyverse`)


```{r ESI-scatter, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
datos_proc %>%  filter(ingresos >= 10000 & ingresos <= 1500000) %>%
   plot_scatter(., ingresos, edad)
```

Recuerden que también podemos incluir una variable de comparación, para eso añadiremos la variable `sexo`

```{r ESI-scatter-sep, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
datos_proc %>%  filter(ingresos >= 10000 & ingresos <= 1500000) %>%
    plot_scatter(., ingresos, edad, sexo,
...
Para los predictores categóricos crearemos gráficos de cajas con la función `plot_grpfrq` de `sjPlot`

```{r ESI-graf-caj, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
plot_grpfrq(datos_proc$ingresos, datos_proc$educacion,
...
```{r ESI-box, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
plot_grpfrq(datos_proc$ingresos, datos_proc$educacion, intr.var = datos_proc$sexo, 
            title = "Gráfico de cajas",
            type = "box")
```


Ahora que observamos como se comportan nuestras variables iremos a la regresión lineal
...
```{r reg, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
...
```{r reg-sj, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
...
```{r pred, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
...
```{r reg-pred-cat, echo=TRUE, message=FALSE, warning=FALSE, results='markup'}
